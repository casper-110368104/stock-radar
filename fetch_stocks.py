...
def fetch_twse_name_lending():
    ...
    for days_back in (0, 1, 2, 3):
        try:
            url = f"https://www.twse.com.tw/rwd/zh/marginTrading/MI_MARGN?date={get_twse_date(days_back)}&selectType=ALL&response=json"
            res = requests.get(url, headers=HEADERS, timeout=15)
            d = res.json()
            ...
            fields = d.get("fields", [])
            rows   = d.get("data",   [])
            print(f"  [TWSE借券] 欄位: {fields}")
            def fi(name, fallback):
                try: return fields.index(name)
                except: return fallback
            # 欄位名稱在不同日期可能不同，多試幾個
            i_code = fi("股票代號", -1)
            if i_code < 0: i_code = fi("證券代號", 0)
            i_name = fi("名稱", -1)
            if i_name < 0: i_name = fi("證券名稱", 1)
...
def fetch_stock_name_industry(sid):
    """從快取取中文名稱，找不到就用 FinMind TaiwanStockInfo 補查"""
    if sid in _name_cache:
        return _name_cache[sid]
    # cache 沒有，用 FinMind 補查（不帶日期）
    try:
        params = {"dataset": "TaiwanStockInfo", "data_id": sid, "token": FINMIND_TOKEN}
        res = requests.get(FINMIND_URL, params=params, timeout=10)
        d = res.json()
        if d.get("status") == 200 and d.get("data"):
            r = d["data"][0]
            name = r.get("stock_name") or r.get("stockName") or ""
            ind  = r.get("industry_category") or r.get("industryCategory") or "其他"
            if name:
