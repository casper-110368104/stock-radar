...
def fetch_securities_lending(sid):
    """抓個股借券賣出（近30日）"""
    try:
        rows = finmind("TaiwanStockSecuritiesLending", sid, get_date(30))
        if not rows:
            return None
        rows.sort(key=lambda x: x.get("date",""))
        latest = rows[-1]
        # 欄位：securities_lending_volume（張）、securities_lending_amount（元）
        vol = int(latest.get("securities_lending_volume", 0) or 0)
        amt = int(latest.get("securities_lending_amount", 0) or 0)
        # 近5日明細
        days5 = []
        seen = set()
        for r in reversed(rows):
            ...
            if d not in seen:
                ...
                days5.append({
                    "date": d,
                    "volume": int(r.get("securities_lending_volume", 0) or 0),
                    "amount": int(r.get("securities_lending_amount", 0) or 0),
                })
            if len(days5) >= 5:
                break
        print(f"      [借券] {sid} 今日={vol}張 金額={amt//10000}萬")
        return {"volume": vol, "amount": amt, "daily": days5}
    except Exception as e:
        print(f"      [借券] {sid} 失敗：{e}")
        return None
...
def fetch_stock_name_industry(sid):
    """從 FinMind TaiwanStockInfo 拿中文名稱和產業（不帶日期參數）"""
    try:
        params = {
        ...
        if status == 200 and data:
            ...
            if name:
                print(f"      [StockInfo] {sid} → {name} / {ind}")
                return name, ind
            print(f"      [StockInfo] {sid} 欄位: {list(r.keys())} 值: {r}")
        else:
            print(f"      [StockInfo] {sid} status={status} data筆數={len(data)} msg={d.get('msg','')} token={'有' if FINMIND_TOKEN else '無'}")
    except Exception as e:
        print(f"      [StockInfo] {sid} 失敗：{e}")
    return sid, "其他"
