...
def fetch_chips(sid):
    ...
    def is_foreign(name):
        n = name.lower()
        return ("外資" in name or "foreign" in n or "qfii" in n) and "自營" not in name and "dealer" not in n
    def is_trust(name):
        n = name.lower()
        return "投信" in name or "investment_trust" in n or "trust" in n
    def is_dealer(name):
        n = name.lower()
        return "自營" in name or "dealer" in n or "proprietary" in n
