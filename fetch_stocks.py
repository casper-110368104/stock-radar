...
def fetch_twse_name_lending():
    ...
    url = f"https://www.twse.com.tw/rwd/zh/marginTrading/MI_MARGN?date={get_twse_date()}&selectType=ALL&response=json"
    try:
        res = requests.get(url, headers=HEADERS, timeout=15)
        d = res.json()
        if d.get("stat") != "OK":
            print(f"  [TWSE名稱借券] 狀態異常: {d.get('stat')}")
            return
        fields = d.get("fields", [])
        rows   = d.get("data",   [])
        print(f"  [TWSE名稱借券] 欄位: {fields[:8]}")
        # 找欄位索引
        def fi(name, fallback):
            try: return fields.index(name)
            except: return fallback
        i_code = fi("股票代號", 0)
        i_name = fi("名稱",     1)
        # 借券相關欄位（欄位名稱因 API 版本不同）
        i_lend_sell = fi("借券賣出", -1)
        i_lend_ret  = fi("借券還券", -1)
        i_lend_bal  = fi("借券餘額", -1)
        # 若找不到，試其他名稱
        if i_lend_bal < 0:
            for alt in ["借券賣出餘額","借券餘額張數","SecLendingBalance"]:
                i_lend_bal = fi(alt, -1)
                if i_lend_bal >= 0: break
        ...
        for r in rows:
            try:
                ...
                if name and code not in _name_cache:
                    _name_cache[code] = (name, "其他")  # 產業之後從 Yahoo 補
                    cnt_name += 1
                if i_lend_bal >= 0 and i_lend_bal < len(r):
                    ...
                    _lending_cache[code] = {"volume": sell, "balance": bal, "return": ret}
                    cnt_lend += 1
            except:
                continue
        print(f"  [TWSE名稱借券] 名稱{cnt_name}檔 借券{cnt_lend}檔")
    except Exception as e:
        print(f"  [TWSE名稱借券] 失敗：{e}")


def fetch_stock_name_industry(sid):
    """從快取取中文名稱和產業（TWSE補名稱，產業從Yahoo）"""
...
def fetch_chips(sid):
    rows = finmind("TaiwanStockInstitutionalInvestorsBuySell", sid, get_date(20))
    if not rows:
        return {}
...
新聞最多3則，sentiment 必須是 bullish/bearish/neutral 之一。"""
    try:
        ...
        if not candidates:
            print(f"    [Gemini] {sid} 無candidates: {rj.get('promptFeedback','')}")
            return [], []
        content = candidates[0].get("content", {})
        parts = content.get("parts", [])
        if not parts:
            print(f"    [Gemini] {sid} 無parts, finishReason={candidates[0].get('finishReason','')}")
            return [], []
...
def main():
    ...
    # 從 TWSE 一次抓全市場名稱+借券（完全不耗 FinMind 額度）
    ...
    for r in results:
        ...
        # 合併借券歷史（保留近7日）
        old_lend_hist = old.get("lending_history", [])
        cur_lend = r.get("lending")
        if cur_lend:
            new_entry = {
                "date":    today,
                "volume":  cur_lend.get("volume",  0),
                "balance": cur_lend.get("balance", 0),
            }
            # 過濾掉今天的舊紀錄（避免重複），再加今天
            hist = [h for h in old_lend_hist if h.get("date") != today]
            hist = sorted(hist + [new_entry], key=lambda x: x["date"], reverse=True)[:7]
            r["lending_history"] = hist
            # 更新 lending.daily 為歷史趨勢
            r["lending"]["daily"] = hist
        else:
            r["lending_history"] = old.get("lending_history", [])
