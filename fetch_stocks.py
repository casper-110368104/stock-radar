...
def calc_score(chips, yahoo, vol_month_ratio, news_list, lending=None, rs_val=None):
    scores = {}

    # ── 籌碼 0~100（單位：萬元）──────────────────────────────
    chip_pts = 0
    f_net = chips.get("foreign", 0)
    f_con = chips.get("foreign_con", 0)  # 連買天數（負=連賣）

    # 外資淨買超（單位：張）
    if   f_net > 3000: chip_pts += 40   # 大買超 3000張+
    elif f_net > 1000: chip_pts += 30
    elif f_net >  300: chip_pts += 20
    elif f_net >   50: chip_pts += 10
    elif f_net >    0: chip_pts += 4
    elif f_net < -300: chip_pts -= 12
    elif f_net <  -50: chip_pts -= 6

    # 外資連買天數加乘（核心改動）
    if f_con >= 15:   chip_pts += 30   # 連買15天以上，強烈多頭
    elif f_con >= 10: chip_pts += 22
    elif f_con >= 7:  chip_pts += 16
    elif f_con >= 5:  chip_pts += 12
    elif f_con >= 3:  chip_pts += 7
    elif f_con >= 1:  chip_pts += 3
    elif f_con <= -10: chip_pts -= 15  # 連賣10天以上，強烈空頭
    elif f_con <= -5:  chip_pts -= 10
    elif f_con <= -3:  chip_pts -= 6
    elif f_con <= -1:  chip_pts -= 3

    # 外資持股比例變化（資料來源待確認，暫不計分）

    # 投信（單位：張）
    t_net = chips.get("trust", 0)
    t_con = chips.get("trust_con", 0)
    if   t_net >  300: chip_pts += 15
    elif t_net >  100: chip_pts += 10
    elif t_net >   20: chip_pts += 6
    elif t_net >    0: chip_pts += 2
    elif t_net < -100: chip_pts -= 8
    if t_con >= 5:    chip_pts += 10
    elif t_con >= 3:  chip_pts += 6
    elif t_con >= 1:  chip_pts += 3
    elif t_con <= -5: chip_pts -= 8
    elif t_con <= -3: chip_pts -= 4

    # 自營商（單位：張）
    d_net = chips.get("dealer", 0)
    if   d_net >  200: chip_pts += 5
    elif d_net >    0: chip_pts += 2
    elif d_net < -200: chip_pts -= 3
    ...
    # 借券餘額絕對值
    if   lend_bal >  5000: chip_pts -= 12
    elif lend_bal >  2000: chip_pts -= 8
    elif lend_bal >   500: chip_pts -= 4
    elif lend_bal ==    0: chip_pts += 3

    # 借券趨勢：近5天餘額是增加還是減少
    if len(lend_hist) >= 3:
        bal_list = [h.get("balance", 0) for h in lend_hist[-3:]]
        if bal_list[-1] > bal_list[0] * 1.2:   chip_pts -= 6  # 近3天增加20%，空頭加碼
        elif bal_list[-1] < bal_list[0] * 0.8: chip_pts += 5  # 近3天減少20%，空頭回補

    # 今日借券賣出
    if   lend_sell > 1000: chip_pts -= 5
    elif lend_sell >  300: chip_pts -= 3
    ...
    if price > 0 and week52h > 0 and week52l >= 0 and week52h > week52l:
        pos = (price - week52l) / (week52h - week52l)  # 0=52週低點, 1=52週高點
        if   pos >= 0.8:  fund_pts += 10   # 接近52週高點，強勢股加分
        elif pos >= 0.6:  fund_pts += 6
        elif pos >= 0.4:  fund_pts += 2
        elif pos <= 0.2:  fund_pts -= 5    # 接近52週低點，弱勢扣分
...
def main():
    ...
    for i, sid in enumerate(all_ids):
        size = yahoo.get("size_cat", "mid")
        cat = "large" if size == "large" else "mid_small"
