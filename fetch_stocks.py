...
def fetch_dynamic_ids():
    """每日動態：量能前100 + 漲幅前50 + 跌幅前30，去重後回傳"""
    try:
        rows = finmind("TaiwanStockPrice", "", get_date(3))
        if not rows: return []
        ...
        # 漲幅前 50（change_percent 或用 spread/open 估）
        def chg(r):
            try:
                o = float(r.get("open", 0) or 0)
                c2 = float(r.get("close", 0) or 0)
                return (c2 - o) / o * 100 if o > 0 else 0
            except: return 0
        ...
        print(f"  動態名單：量能{len(top_vol)} + 漲幅{len(top_rise)} + 跌幅{len(top_fall)} → 去重後 {len(merged)} 檔")
        return merged
    except Exception as e:
