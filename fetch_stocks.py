...
def fetch_shareholder(sid):
    """從 Goodinfo 抓四大法人持股比：外資/投信/自營/散戶"""
    url = f"https://goodinfo.tw/tw/StockShareholderForm.asp?STOCK_ID={sid}"
    hdrs = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
        "Accept-Language": "zh-TW,zh;q=0.9",
        "Referer": "https://goodinfo.tw/tw/",
    }
    empty = {"foreign": None, "trust": None, "dealer": None, "retail": None, "trend": []}
    try:
        ...
        # Goodinfo 防爬檢查
        if "請勿透過" in html or "robot" in html.lower():
            print(f"      [Goodinfo] {sid} 被擋")
            return empty

        from bs4 import BeautifulSoup
        soup = BeautifulSoup(html, "html.parser")

        # 找持股比資料表（通常在 id=tblDetail 或含「外資」的 table）
        result = dict(empty)
        tables = soup.find_all("table")
        for tbl in tables:
            text = tbl.get_text()
            if "外資" in text and "投信" in text and "自營" in text:
                ...
                for row in rows:
                    ...
                    if "外資" in label and "合計" not in label and result["foreign"] is None:
                        result["foreign"] = safe_float(cells[1]) if len(cells)>1 else None
                    elif "投信" in label and result["trust"] is None:
                        result["trust"] = safe_float(cells[1]) if len(cells)>1 else None
                    elif "自營" in label and "合計" not in label and result["dealer"] is None:
                        result["dealer"] = safe_float(cells[1]) if len(cells)>1 else None
                    elif any(k in label for k in ["散戶","一般","個人"]) and result["retail"] is None:
                        result["retail"] = safe_float(cells[1]) if len(cells)>1 else None
                if result["foreign"] is not None:
                    break

        # 散戶 = 100 - 其他三者（若無直接資料）
        if result["retail"] is None:
            vals = [v for v in [result["foreign"], result["trust"], result["dealer"]] if v is not None]
            if vals:
                result["retail"] = round(100 - sum(vals), 2)

        print(f"      [Goodinfo] {sid} 外資{result['foreign']}% 投信{result['trust']}% 自營{result['dealer']}% 散戶{result['retail']}%")
        return result

    except Exception as e:
        print(f"      [Goodinfo] {sid} 失敗：{e}")
        return empty


def fetch_stock_name_industry(sid):
    """從 FinMind TaiwanStockInfo 拿中文名稱和產業"""
    try:
        rows = finmind("TaiwanStockInfo", sid, "2020-01-01")
        for r in rows:
            if r.get("stock_id") == sid:
                return r.get("stock_name", sid), r.get("industry_category", "其他")
    except:
        pass
    return sid, "其他"
...
def fetch_chips(sid):
    ...
    if latest:
        print(f"      [籌碼欄位] {list(latest[0].keys())}")
        print(f"      [籌碼範例] {latest[0]}")

    def is_foreign(name):
        return ("外資" in name or "Foreign" in name or "foreign" in name) and "自營" not in name and "Dealer" not in name
    def is_trust(name):
        return "投信" in name or "Investment_Trust" in name or "investment_trust" in name
    def is_dealer(name):
        return "自營" in name or "Dealer" in name or "dealer" in name
...
def process_stock(sid, category):
    ...
    # 用 FinMind 取中文名稱和產業（覆蓋 Yahoo 英文）
    ...
    print("持股...", end=" ", flush=True)
    holding = fetch_shareholder(sid)
    time.sleep(0.3)
