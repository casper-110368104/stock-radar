...
# 台灣上市類股分類（Yahoo 台股標準）
TW_INDUSTRIES = [
    "半導體","電腦及週邊","電子零組件","光電","通信網路","其它電子",
    "電子通路","資訊服務","數位雲端","電機機械","電器電纜",
    "化學工業","生技醫療","玻璃陶瓷","造紙","鋼鐵","橡膠",
    "汽車","水泥","食品","紡織","塑膠化工","水泥窯製",
    "航運","觀光餐旅","金融","貿易百貨","建材營造",
    "油電燃氣","綠能環保","居家生活","運動休閒","其他"
]

def fetch_all_industries(sids):
    """一次呼叫 Gemini 判斷所有股票的中文產業分類"""
    if not GEMINI_API_KEY:
        print("  [產業] 無 Gemini Key，跳過")
        return

    need = {}
    for sid in sids:
        name, ind = _name_cache.get(sid, (sid, "其他"))
        is_chinese = any('\u4e00' <= ch <= '\u9fff' for ch in (ind or ""))
        if not is_chinese or ind == "其他":
            need[sid] = name

    if not need:
        print("  [產業] 所有股票已有中文產業，跳過")
        return

    print(f"  [產業] 傳送 {len(need)} 支股票給 Gemini 一次判斷...")
    ind_list = "\n".join(TW_INDUSTRIES)
    stock_lines = "\n".join([f"{sid} {name}" for sid, name in need.items()])
    prompt = (
        "以下是台灣上市股票代碼和名稱清單。\n\n"
        "【重要】產業 = 公司主要營業項目（如：台積電→半導體、鴻海→電零組件、國泰金→金融）\n"
        "概念股題材（AI伺服器、CoWoS、低軌衛星等）不是產業，請勿選\n\n"
        f"產業清單（只能從這裡選，不能自創）：\n{ind_list}\n\n"
        f"股票清單：\n{stock_lines}\n\n"
        '回傳格式（只回JSON，不要markdown，不要說明文字）：\n{"2330":"半導體","2412":"通信網路",...}\n'
        "每支股票選一個最符合主業的產業。"
    )
    safety = [
    ...
    for model in models:
        try:
            ...
            rj = res.json()
            if "error" in rj:
                print(f"  [產業] {model} 錯誤: {rj['error'].get('message','')[:80]}")
                continue
            text = rj["candidates"][0]["content"]["parts"][0]["text"]
            text = text.replace("```json","").replace("```","").strip()
            result = json.loads(text)
            ok = 0
            for sid, ind in result.items():
                if sid in _name_cache and ind in TW_INDUSTRIES:
                    name = _name_cache[sid][0]
                    _name_cache[sid] = (name, ind)
                    ok += 1
            print(f"  [產業] {model} ✓ 更新 {ok}/{len(need)} 檔")
            return
        except Exception as e:
            print(f"  [產業] {model} 失敗：{e}")
            continue
    print("  [產業] 所有模型失敗，使用原有產業")
