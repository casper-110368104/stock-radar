...
def calc_score(chips, yahoo, vol_month_ratio, news_list, lending=None, rs_val=None, rs_pct=None, revenue_yoy=None):
    ...
    f_con = chips.get("foreign_con", 0)
    vol_lots = (vol_today / 1000) if vol_today > 0 else 1

    # 外資買超佔成交量%
    f_pct = (f_net / vol_lots * 100) if vol_lots > 0 else 0
    if   f_pct > 10: chip_pts += 40
    elif f_pct >  5: chip_pts += 25
    elif f_pct >  2: chip_pts += 15
    ...
    t_pct = (t_net / vol_lots * 100) if vol_lots > 0 else 0
    if   t_pct >  3: chip_pts += 15
    elif t_pct >  1: chip_pts += 8
    elif t_pct >  0: chip_pts += 3
    elif t_pct < -1: chip_pts -= 6
    if   t_con >= 5: chip_pts += 8
    ...
    d_pct = (d_net / vol_lots * 100) if vol_lots > 0 else 0
    if   d_pct >  2: chip_pts += 4
    elif d_pct >  0: chip_pts += 2
    elif d_pct < -2: chip_pts -= 3
    ...
    shares_lots = (shares / 1000) if shares > 0 else 0
    lend_pct = (lend_bal / shares_lots * 100) if shares_lots > 0 else 0
    if   lend_pct >  8: chip_pts -= 15
    elif lend_pct >  5: chip_pts -= 10
    elif lend_pct >  3: chip_pts -= 6
    elif lend_bal == 0: chip_pts += 3

    # 借券趨勢（近3天）
    if len(lend_hist) >= 3:
        bl = [h.get("balance", 0) for h in lend_hist[-3:]]
        if bl[-1] > bl[0] * 1.2:   chip_pts -= 5
        elif bl[-1] < bl[0] * 0.8: chip_pts += 4

    # 借券賣出佔比
    sell_pct = (lend_sell / vol_lots * 100) if vol_lots > 0 else 0
    if   sell_pct > 3: chip_pts -= 5
    ...
    if   eg > 50:  fund_pts += 35
    elif eg > 30:  fund_pts += 28
    elif eg > 20:  fund_pts += 22
    elif eg > 10:  fund_pts += 15
    elif eg >  5:  fund_pts += 10
    elif eg >  0:  fund_pts += 5
    elif eg > -10: fund_pts += 0
    elif eg > -30: fund_pts -= 8
    else:          fund_pts -= 15

    if   roe > 25: fund_pts += 30
    elif roe > 20: fund_pts += 24
    elif roe > 15: fund_pts += 18
    elif roe > 10: fund_pts += 10
    elif roe >  5: fund_pts += 4
    elif roe <= 0: fund_pts -= 8

    if   dr < 20: fund_pts += 20
    elif dr < 30: fund_pts += 15
    elif dr < 40: fund_pts += 10
    elif dr < 60: fund_pts += 4
    elif dr > 80: fund_pts -= 8
    ...
    if price > 0 and week52h > 0 and week52l > 0:
        # 52週新高突破
        ratio = price / week52h
        if   ratio >= 1.00: tech_pts += 20
        elif ratio >= 0.95: tech_pts += 12
        elif ratio >= 0.85: tech_pts += 6
        elif ratio >= 0.70: tech_pts += 2
        elif ratio <  0.40: tech_pts -= 6
        # 52週回撤
        drawdown = (week52h - price) / week52h
        if   drawdown < 0.10: tech_pts += 12
        elif drawdown < 0.20: tech_pts += 8
        elif drawdown < 0.35: tech_pts += 3
        elif drawdown > 0.50: tech_pts -= 10
        if   drawdown > 0.70: tech_pts -= 5
        # 52週波動率
        vol52 = (week52h - week52l) / week52l
        if   vol52 > 1.50: tech_pts += 12
        elif vol52 > 1.00: tech_pts += 8
        elif vol52 > 0.60: tech_pts += 4
        elif vol52 < 0.30: tech_pts -= 5
    scores["technical"] = max(0, min(tech_pts, 100))

    # ── 量能 0~100（vol20/vol60）─────────────────────────────
    vol20 = yahoo.get("vol20_avg") or 0
    vol60 = yahoo.get("vol60_avg") or 0
    if vol20 > 0 and vol60 > 0:
        vr = vol20 / vol60
        if   vr > 2.0: vol_pts = 100
        elif vr > 1.5: vol_pts = 80
        elif vr > 1.3: vol_pts = 60
        elif vr > 1.1: vol_pts = 40
        elif vr > 1.0: vol_pts = 20
        else:          vol_pts = 0
    else:
        if   vol_month_ratio > 2.0: vol_pts = 100
        elif vol_month_ratio > 1.5: vol_pts = 80
        elif vol_month_ratio > 1.2: vol_pts = 60
    ...
    if revenue_yoy is not None:
        ...
        elif revenue_yoy > 20: rev_pts = 80
        elif revenue_yoy > 10: rev_pts = 60
        elif revenue_yoy >  5: rev_pts = 40
        elif revenue_yoy >  0: rev_pts = 20
        else:                   rev_pts = max(0, 20 + revenue_yoy * 2)
    ...
    if rs_pct is not None:
        ...
        elif rs_pct >= 80: scores["rs"] = 80
        elif rs_pct >= 70: scores["rs"] = 60
        elif rs_pct >= 50: scores["rs"] = 40
        else:              scores["rs"] = max(0, int(rs_pct * 0.6))
    elif rs_val is not None:
        if   rs_val >  0.30: scores["rs"] = 100
        elif rs_val >  0.15: scores["rs"] = 80
        elif rs_val >  0.05: scores["rs"] = 60
        elif rs_val >  0:    scores["rs"] = 45
        elif rs_val > -0.05: scores["rs"] = 30
        elif rs_val > -0.15: scores["rs"] = 15
        else:                scores["rs"] = 0
    else:
        scores["rs"] = 0
...
def process_stock(sid, category):
    ...
    scores     = calc_score(chips, yahoo, vol_month_ratio, news, lending, rs_val)
